...
import { linesToArray, slugify } from "@/lib/content";
...
function parse(formData: FormData) {
  ...
  return {
    data: { name, slug, region, tagline, description, heroImage, gallery, highlights, duration, difficulty, bestSeason, priceFrom, sortOrder, featured, published },
  } as const;
}
...
export async function updateDestination(id: string, _prev: FormState, formData: FormData): Promise<FormState> {
  ...
  const before = await prisma.destination.update({ where: { id }, data: parsed.data });
